...
export const HAND_STRENGTH: Record<string, number> = {
  straight_flush: 900, four: 800, full_house: 700, flush: 600, straight: 500,
  three: 400, two_pair: 300, pair: 200, high_card: 100, flush_draw: 80,
  straight_draw: 70, garbage: 10,
};
...
function tiebreak(distinctDesc: number[], byRank: Map<number, number>): number {
  ...
  let s = 0;
  let place = 1;
  for (const [v] of ranked) { s += v * place; place *= 0.01; }
  for (const v of distinctDesc) { s += v * place; place *= 0.01; }
  return s;
}
...
export function classifyPlayed(cards: CardRef[]): { handType: MadeHandType; strength: number } {
  if (cards.length === 0) return { handType: "high_card", strength: HAND_STRENGTH.high_card };
  ...
  if (isFlush && isStraight) return { handType: "straight_flush", strength: HAND_STRENGTH.straight_flush + kick };
  if (counts[0] === 4) return { handType: "four", strength: HAND_STRENGTH.four + kick };
  if (counts[0] === 3 && counts[1] === 2) return { handType: "full_house", strength: HAND_STRENGTH.full_house + kick };
  ...
  if (counts[0] === 2 && counts[1] === 2) return { handType: "two_pair", strength: HAND_STRENGTH.two_pair + kick };
...
function toCombo(cards: CardRef[], handType: string, strength: number, made: boolean): RankedCombo {
  const sorted = [...cards].sort((a, b) => rankValue(b.rank) - rankValue(a.rank) || a.index - b.index);
  const indices = sorted.map((c) => c.index);
  const tokens = sorted.map(cardToken);
  return {
    indices, handType, strength, cardsKey: tokens.join("_"),
    label: `${handType.replace(/_/g, " ")}: ${tokens.join(" ")}`, made,
  };
}
...
export function enumeratePlayCombos(hand: CardRef[]): RankedCombo[] {
  ...
  for (let k = 1; k <= maxPlay; k++) {
    for (const subset of combinations(hand, k)) {
      ...
      if (handType === "high_card" && k !== 1 && k !== Math.min(5, hand.length)) continue;
  ...
  for (const list of bySuit.values()) {
    if (list.length >= 4 && list.length < 5) {
      const top = [...list].sort((a, b) => rankValue(b.rank) - rankValue(a.rank)).slice(0, 4);
      consider(toCombo(top, "flush_draw", HAND_STRENGTH.flush_draw + rankValue(top[0]!.rank) * 0.1 + list.length, false));
    }
    if (list.length >= 5) {
      const top5 = [...list].sort((a, b) => rankValue(b.rank) - rankValue(a.rank)).slice(0, 5);
      const { handType, strength } = classifyPlayed(top5);
      consider(toCombo(top5, handType, strength, true));
    }
  }
  const distinctCards = [...hand].sort((a, b) => rankValue(a.rank) - rankValue(b.rank));
  for (const subset of combinations(distinctCards, 4)) {
    const vals = uniqueSorted(subset.map((c) => rankValue(c.rank)));
    if (vals.length !== 4) continue;
    let ok = true;
    for (let i = 1; i < vals.length; i++) {
      if (vals[i]! - vals[i - 1]! > 2) { ok = false; break; }
    }
    const span = vals[vals.length - 1]! - vals[0]!;
    if (ok && span <= 4) {
      consider(toCombo(subset, "straight_draw", HAND_STRENGTH.straight_draw + vals[vals.length - 1]! * 0.1, false));
    }
  }
  const topCards = [...hand].sort((a, b) => rankValue(b.rank) - rankValue(a.rank)).slice(0, Math.min(5, hand.length));
  {
    const { handType, strength } = classifyPlayed(topCards);
    consider(toCombo(topCards, handType, strength, handType !== "high_card"));
  }
  {
    const best = topCards[0]!;
    consider(toCombo([best], "high_card", HAND_STRENGTH.high_card + rankValue(best.rank), false));
  }
  const all = [...bestByKey.values()].sort((a, b) => b.strength - a.strength);
  const kept: RankedCombo[] = [];
  const seenTypes = new Set<string>();
  for (const c of all) {
    if (c.made && !seenTypes.has(c.handType)) { kept.push(c); seenTypes.add(c.handType); }
  }
  for (const c of all) {
    if (kept.includes(c)) continue;
    if (!c.made && (c.handType === "flush_draw" || c.handType === "straight_draw")) {
      const drawKey = `${c.handType}_${c.cardsKey}`;
      if (!seenTypes.has(drawKey)) { kept.push(c); seenTypes.add(drawKey); }
    }
  }
  for (const c of all) {
...
export function enumerateDiscardCombos(hand: CardRef[]): RankedCombo[] {
  ...
  const bestDraw = plays.find((p) => p.handType === "flush_draw" || p.handType === "straight_draw");
  const keepSet = new Set(
    (bestMade ?? bestDraw)?.indices ??
      [...hand].sort((a, b) => rankValue(b.rank) - rankValue(a.rank)).slice(0, 2).map((c) => c.index),
  );
  ...
  const pushDead = (cards: CardRef[], reason: string, strength: number) => {
    ...
    out.push(toCombo(sorted.slice(0, Math.min(5, sorted.length)), reason, strength, false));
  };
  if (dead.length > 0) {
    pushDead(dead, "dead_cards", 50 + dead.length);
    const lowest = [...dead].sort((a, b) => rankValue(a.rank) - rankValue(b.rank));
  ...
  for (const [suit, list] of bySuit) {
    if (list.length > bestSuitCount) { bestSuitCount = list.length; bestSuit = suit; }
  }
  if (bestSuit && bestSuitCount >= 3) {
    pushDead(hand.filter((c) => c.suit !== bestSuit), "off_suit", 55 + bestSuitCount);
  }
...
export function pickBestHandAction(
  ...
  const strongMade = plays.find((p) =>
    p.made && ["straight_flush", "four", "full_house", "flush", "straight", "three", "two_pair", "pair"].includes(p.handType),
  );
  const bestDraw = plays.find((p) => p.handType === "flush_draw" || p.handType === "straight_draw");
  if (opts.handsLeft > 0 && strongMade) return { kind: "play_hand", combo: strongMade };
  if (opts.discardsLeft > 0 && !hasMade && bestDraw && discards[0]) return { kind: "discard", combo: discards[0] };
  if (opts.handsLeft > 0) return { kind: "play_hand", combo: bestPlay };
  if (opts.discardsLeft > 0 && discards[0]) return { kind: "discard", combo: discards[0] };
  return { kind: "play_hand", combo: bestPlay };
}

export function actionId(kind: "play" | "discard", combo: RankedCombo): string {
  return `${kind}_${combo.handType}_${combo.cardsKey}`.replace(/[^a-zA-Z0-9_]/g, "").slice(0, 80);
}
