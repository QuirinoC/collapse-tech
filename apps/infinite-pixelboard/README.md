...
The `/moderation` page is an authentication-gated client shell; it reveals no private controls or data until Firebase has refreshed an ID token containing the exact custom claim `moderator=true`. Every `/api/v1/moderation` route enforces that claim server-side. The routes expose the report queue and server-authored evidence plus audited, idempotent dismiss, quarantine, rollback, warn, suspend, ban, placement-freeze, and ad-shutdown actions. Quarantine masks active regions from public tile snapshots. Rollback only changes selected placements that are still current and restores the prior color and ownership state. Moderators can also create special redeemable codes via `POST /api/v1/moderation/special-codes` (see the moderation runbook for a sample unlimited-placing curl).
